 ...
 *   - each time the key of a property matches the key of a transform,
 ...
 *   - transforms input can be augmented by using option `args(input)`
 *     which must return extra input as an object
 *     Can also be a simple object instead of a function
 *   - transforms must return an object containing the properties to assign
 *     to the property's parent.
 *     Note this is assigned to the parent, not to the property itself
 *     They can return undefined to signify "no change"
 *     If a value inside the returned object is undefined however,
 *     it means "remove this property"
 *   - a transform called `any` will always be triggered at the end
 *     of a given pass
 * Transforms are an array of maps:
 *  - each map is processed after another, i.e. transformation can be applied
 *    in well-defined passes
