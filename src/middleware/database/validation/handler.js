...
const dataValidation = function ({ args, modelName, command, idl, mInput }) {
  const schema = getDataValidationSchema({ idl, modelName, command });
  const attrs = getAttrs({ args });

  Object.entries(attrs).forEach(
    ([dataVar, attr]) => validateAttr({ idl, dataVar, attr, schema, mInput }),
  );
};
...
const validateAttr = function ({ idl, dataVar, attr, schema, mInput }) {
  const attrArray = Array.isArray(attr) ? attr : [attr];

  return attrArray.forEach(
    data => validateSingleAttr({ idl, dataVar, schema, mInput, data }),
  );
};

const validateSingleAttr = function ({ idl, dataVar, schema, mInput, data }) {
  const dataA = removeEmpty(data);

  const reportInfo = { type, dataVar };

  validate({ idl, schema, data: dataA, reportInfo, extra: mInput });
};
