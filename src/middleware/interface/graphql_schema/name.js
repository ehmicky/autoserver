...
const getTypeName = function ({ def, opts: { inputObjectType, action: { actionType = '', multiple } = {} } }) {
  ...
  return camelize(capitalize(`${actionType} ${name} ${inputObjectType}${nestedPostfix}`));
};

// Returns action name, camelized, in plural form, e.g. `findPets` or `deletePets`
const getActionName = function ({ modelName, actionType, multiple }) {
  const model = multiple === true ? plural(modelName) : multiple === false ? singular(modelName) : modelName;
  return camelize(`${actionType} ${model}`);
};
