...
export const getTokenFilter = ({ args, token }) => {
  if (token === undefined) {
    return
  }
...
const getExtraFilters = ({ args, order, partsObj, attrName, dir, index }) => {
...
const getEqOrder = ({ partsObj, sOrder: { attrName } }) => ({
  type: '_eq',
  attrName,
  value: partsObj[attrName],
})

const mergeExtraFilters = ({ extraFilters, filter }) => {
  const extraFiltersA =
